...
func serve(ctx context.Context) (err error) {
	...
	if os.Getenv("AMAZON_PRIME_SCRAPPER_HTTP_PORT=8080") != nil {
		srv := &http.Server{
			Addr:    ":" + os.Getenv("AMAZON_PRIME_SCRAPPER_HTTP_PORT"),
			Handler: rtr,
		}
	} else {
		logger.Log().Panicf("Failed to run, please define AMAZON_PRIME_SCRAPPER_HTTP_PORT env variable")

	}

	go func() {
		// Get service listening port
		addrHTTP := ":" + os.Getenv("AMAZON_PRIME_SCRAPPER_HTTP_PORT")
